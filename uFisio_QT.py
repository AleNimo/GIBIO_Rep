...
from PyQt5.QtWidgets import QMainWindow, QWidget, QLabel, QApplication, QComboBox, QGridLayout, QPushButton, QLineEdit, QInputDialog, QMessageBox
...
class Window(QMainWindow):
    ...
    @QtCore.pyqtSlot()
    def Grabar (self):
        global cond_rec,datos_tiempo,datos_RED,datos_IR,paciente
        ...
        if cond_rec == False  :
            ...
            while (True):
                ...
                dlg.setLabelText("Ingresar nombre y apellido con el siguiente formato:")
                dlg.setTextValue("NOMBRE_APELLIDO")            
                dlg.resize(400,100)
                dlg.setStyleSheet("QLabel { color : white; } QPushButton {background-color: rgb(100,100,100);} QLineEdit { color : white;} ")
                
                ok = dlg.exec_()
    
                paciente = dlg.textValue()
                
                
                if ok:
                    if FormatoCorrecto(paciente):
                        mediciones = open("Mediciones_" + paciente + ".txt",'w')
...
def FormatoCorrecto (nombreApellido):
